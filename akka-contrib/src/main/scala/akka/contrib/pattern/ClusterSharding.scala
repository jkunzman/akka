 ...
 * which might also change over time. It can be actors representing Aggregate Roots in
 ...
 * continue until the handoff is completed. The `ShardRegion` that owns the rebalanced shard
 ...
 * persistent (durable), e.g. with `akka-persistence`, so that it can be recovered on the new
 * location.
 *
 ...
 * between it received `Passivate` and the entry is terminated. Such buffered messages
...
class ClusterSharding(system: ExtendedActorSystem) extends Extension {
   ...
   *   be `unhandled`, i.e. sent to `deadLetters`
...
object ShardRegion {
   ...
   * `unhandled`, i.e. sent to `deadLetters`. Note that the extracted
   * message does not have to be the same as the incoming message to
   * support wrapping in message envelope that is unwrapped before
  ...
  trait MessageExtractor {
     ...
     * the message will be `unhandled`, i.e. sent to `deadLetters`
     */
...
object ShardCoordinator {
  ...
  private[akka] object Internal {
    ...
    /**
     * Acknoledgement from `ShardCoordinator` that [[Register]] or [[RegisterProxy]] was sucessful.
     */
     ...
     * supposed to discard their know location of the shard, i.e. start buffering
    ...
    /**
     * Acknoledgement of [[BeginHandOff]]
     */
