...
As a rule of thumb, use the same amount of shards as around 10 times the maximum number of planned
cluster nodes.
...
#. Thereafter all incoming messages to R1 for S1 can be handled by R1 without C. It creates entry children as needed, and forwards messages to them.
...
#. Thereafter all incoming messages to R1 for S2 can be handled by R1 without C. It forwards messages to R2.
